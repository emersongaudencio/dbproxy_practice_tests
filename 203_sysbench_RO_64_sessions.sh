...
sysbench /usr/share/sysbench/oltp_read_only.lua --threads=64 --events=0 --time=300 --mysql-host=${VAR_HOST} --mysql-user=app_user --mysql-password=test123 --mysql-port=${VAR_PORT} --tables=${VAR_TABLE_COUNT}  --table-size=10000 --db-ps-mode=disable --report-interval=1 --mysql-ignore-errors=all run
